...
# CN2Global 完整部署脚本 - 可直接在服务器上运行
# 使用方法：bash deploy_final.sh
...
if [ "$EUID" -ne 0 ]; then 
    ...
    echo "使用: sudo bash deploy_final.sh"
    exit 1
fi
...
log_success "应用目录已创建: /var/www/cn2global"
...
log_warning "请稍后编辑 .env 文件，添加 Google OAuth 凭证"
...
certbot certonly --nginx -d cn2g.com -d www.cn2g.com --non-interactive --agree-tos -m admin@cn2g.com 2>&1 || log_warning "SSL 证书申请可能失败，请稍后手动申请"
